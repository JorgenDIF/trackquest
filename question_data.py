"""Contains all questions and answers for the game.
The questions are mostly translations from the Swedish version of the game.
Credits to https://pasparetbloggen.blogspot.com/p/blog-page.html
"""
questions = [

    # City 1 questions

    {
        "category": "city_1",
        "points": 10,
        "text": (
            "Build your city on a rock, said the wise, but our city is built "
            "on sand from another continent. In our city, there's a Don who "
            "is not a godfather, rather the opposite but he still "
            "drives a Ferrari."
        ),
        "answer": "Miami",
    },
    {
        "category": "city_1",
        "points": 6,
        "text": (
            "Bad boys are doing their best to serve society in our city. "
            "But maybe it's the girls gone wild during spring break that "
            "made Sisqo want to make TV here."
        ),
        "answer": "Miami",
    },
    {
        "category": "city_1",
        "points": 4,
        "text": (
            "Our city has seen many peculiar fictional murders carried out "
            "by a serial killer who also investigates his own murders. "
            "Spanish is a second language in our American city, what do "
            "you say about that, mi-amigo? Despite Spanish, it's a French "
            "design style that made the beach famous."
        ),
        "answer": "Miami",
    },
    {
        "category": "city_1",
        "points": 2,
        "text": (
            "If you want to eat oranges and retire, you've come to "
            "the right place, "
            "Key West and South Beach are neighbors with our city, so relax, "
            "you're now in ____!"
        ),
        "answer": "Miami",
    },

    # City 2 questions

    {
        "category": "city_2",
        "points": 10,
        "text": (
            "Where are we heading? We leave "
            "zebra-inspired architecture in one "
            "of the country's oldest, heading towards a city where a lord has "
            "added a starting city. "
            "Indie pop is a clue that fits like a glove."
        ),
        "answer": "Manchester",
    },
    {
        "category": "city_2",
        "points": 8,
        "text": (
            "Balter resides at one end of our city, "
            "a heavily loaded German at "
            "the other, and in the middle stands "
            "a red revolutionary. A Dutchman "
            "steers the city's red, while a "
            "Spanish administrator manages the blue."
        ),
        "answer": "Manchester",
    },
    {
        "category": "city_2",
        "points": 6,
        "text": (
            "Remove the short name from the "
            "42-kilometer race from the tattooed "
            "girl, and you get a football "
            "profile in our city. Frozen water from "
            "a desert paradise has ties to the "
            "city where the suffragette movement "
            "was founded."
        ),
        "answer": "Manchester",
    },
    {
        "category": "city_2",
        "points": 4,
        "text": (
            "Freshly groomed slopes and striped cotton fabric bring you this "
            "industrial city situated between "
            "Liverpool and Leeds. We're looking "
            "for the hometown of The Smiths, Oasis, and Joy Division."
        ),
        "answer": "Manchester",
    },
    {
        "category": "city_2",
        "points": 2,
        "text": (
            "We have now arrived in England's third, "
            "where one begins, and where Ester concludes."
        ),
        "answer": "Manchester",
    },

    # City 3 questions

    {
        "category": "city_3",
        "points": 10,
        "text": (
            "Where are we heading? We leave the erudite in a cattle cart and "
            "follow the river towards a distant smoke in the east. "
            "In the next harbor, the destination is also "
            "the point of departure. "
            "Through the city's eye, you see its ominous center,"
            "where swift danger will never be forgotten."
        ),
        "answer": "London",
    },
    {
        "category": "city_3",
        "points": 8,
        "text": (
            "Bond's fairytale bear has its own station here, "
            "as well as a renowned Belgian battlefield. "
            "On the wet streets, double decks have become standard, "
            "both on wet and dry surfaces. "
            "If you consult the route master, you'll find your way to "
            "a South Spanish water war and a residence for three feet."
        ),
        "answer": "London",
    },
    {
        "category": "city_3",
        "points": 6,
        "text": (
            "In 2012, this was the blue and yellow sailing triumph. "
            "The creator of sailing was born here. But tell me, "
            "have Rod's ventures been successful? "
            "We are soon arriving in a colossal tourist city, "
            "and one could playfully say that the city calls."
        ),
        "answer": "London",
    },
    {
        "category": "city_3",
        "points": 4,
        "text": (
            "In the arsenal, several substantial pieces with gunpowder in the "
            "barrel have been active; one was named Fredrik, another Thierry."
            "\nTell me, has Big Ben become any wiser with time?"
            "\nNo, he's 'anciently unwise.'"
            "\nHaha, that was said with perfect timing."
        ),
        "answer": "London",
    },
    {
        "category": "city_3",
        "points": 2,
        "text": (
            "We've arrived in Elizabeth II's hometown and the host city for "
            "the 2012 Olympics, where shopping takes a pause "
            "a bit of a 'low, Don' moment, "
            "reflecting the whimsy that this city is known for."
        ),
        "answer": "London",
    },

    # City 4 questions

    {
        "category": "city_4",
        "points": 10,
        "text": (
            "Where are we heading? We leave poetic meters on the west "
            "side and journey towards the east side, to a city where "
            "you also call home. "
            "We're heading to the capital, but for that, you must "
            "signal left in the roundabout."
        ),
        "answer": "Dublin",
    },
    {
        "category": "city_4",
        "points": 8,
        "text": (
            "We find ourselves in a realm where infrared "
            "radiation is of paramount importance, "
            "and where a gilded string instrument is the primary weapon. "
            "The city we seek has "
            "connections to a Trojan hero, but if you live "
            "wildly here, you engage in hurling."
        ),
        "answer": "Dublin",
    },
    {
        "category": "city_4",
        "points": 6,
        "text": (
            "Dancing the river dance might be a humble suggestion "
            "in our city, but while waiting "
            "to reach this literary metropolis, you can partake "
            "in the black gold; one is yeast, "
            "and the other is spiked."
        ),
        "answer": "Dublin",
    },
    {
        "category": "city_4",
        "points": 4,
        "text": (
            "We travel to the southern part of the Emerald "
            "Isle where the patron saint is named "
            "Patrick. Johnny's slogan has brought musical success "
            "to the country, and so have you."
        ),
        "answer": "Dublin",
    },
    {
        "category": "city_4",
        "points": 2,
        "text": (
            "Blindly trusting these clues is not recommended; "
            "you might be pleasantly surprised. "
            "Instead, think of the capital of Ireland."
        ),
        "answer": "Dublin",
    },

    # City 5 questions

    {
        "category": "city_5",
        "points": 10,
        "text": (
            "Where are we heading? We leave the former book collection "
            "and journey south through a pentagonal land. "
            "In the central parts of our city, the air is polluted."
        ),
        "answer": "Kairo",
    },
    {
        "category": "city_5",
        "points": 8,
        "text": (
            "Pasternak's original doctor hails from here, where coffee "
            "breaks in a calming environment "
            "are never in question, but a falafel will do just fine."
        ),
        "answer": "Kairo",
    },
    {
        "category": "city_5",
        "points": 6,
        "text": (
            "Anwar said that, Caesar's wife said something "
            "entirely different. Our city "
            "is a true cultural hub, and "
            "many brilliant minds reside here."
        ),
        "answer": "Kairo",
    },
    {
        "category": "city_5",
        "points": 4,
        "text": (
            "The country is deeply involved in Islam, and one "
            "of the world's seven wonders is not far away, Giza, that is."
        ),
        "answer": "Kairo",
    },
    {
        "category": "city_5",
        "points": 2,
        "text": (
            "Grab your map and hop into the car because we're going on a tour"
            " to... Pharaoh too, Kai is rowing in the wrong direction."
        ),
        "answer": "Kairo",
    },

    # City 6 questions

    {
        "category": "city_6",
        "points": 10,
        "text": (
            "Where are we heading? We depart from a fortified city in "
            "the south, whose initials also initiate "
            "a region, the fifth city of which is our target. On our "
            "journey northward, we pass beach after "
            "beach before reaching our coastless city with significant allure."
        ),
        "answer": "Orlando",
    },
    {
        "category": "city_6",
        "points": 8,
        "text": (
            "Knowledge of the 35th president of the United States is "
            "advantageous, as this spaced-out information "
            "guides you to the neighboring city, 8 miles east of our "
            "orange-hued city with a strong interest in golf."
        ),
        "answer": "Orlando",
    },
    {
        "category": "city_6",
        "points": 6,
        "text": (
            "Jurassic Park, E.T., and King Kong are universal "
            "film references of a wholly different character than "
            "the elf from Mirkwood, but all lead to the city where "
            "both Rwanda and Romania are central concepts."
        ),
        "answer": "Orlando",
    },
    {
        "category": "city_6",
        "points": 4,
        "text": (
            "Used golf balls and a Puch model evoke thoughts of "
            "our state in the southeast. "
            "If you replace one of the flanking 'a's with an 'o' "
            "in the blue and yellow airport, you've landed correctly."
        ),
        "answer": "Orlando",
    },
    {
        "category": "city_6",
        "points": 2,
        "text": (
             "Disney World, Sea World, and Universal Studios attract "
             "all tourists to this Florida city that starts with an O."

         ),
        "answer": "Orlando",
    },

    # City 7 questions

    {
        "category": "city_7",
        "points": 10,
        "text": (
            "Where are we heading? We leave a famous trial in the west "
            "and head east towards a city where a legally protected "
            "sacred scripture originated. The gravestone for "
            "livestock holds economic significance here."
        ),
        "answer": "Prag",
    },
    {
        "category": "city_7",
        "points": 8,
        "text": (
            "In this literary metropolis, America cuts to the chase, "
            "where upholstery fabric has revolutionized the country."
        ),
        "answer": "Prag",
    },
    {
        "category": "city_7",
        "points": 6,
        "text": (
            "Our city is old and beautiful, especially in spring. "
            "On Kalle's Bridge, the Swedish army once fought, "
            "and Charta 77 may have its origins here."
        ),
        "answer": "Prag",
    },
    {
        "category": "city_7",
        "points": 4,
        "text": (
            "I am sure that the country is good at ice hockey; the city, "
            "in any case, is the best in the world at PR. "
            "Have a drink of the country's famous beer, "
            "and you'll surely get hit on."
        ),
        "answer": "Prag",
    },
    {
        "category": "city_7",
        "points": 2,
        "text": (
            "In our Central European country, once united with "
            "Slovakia, one can pay with checks again. "
            "Now we put a 'P' on it this time!"
        ),
        "answer": "Prag",
    },

    # City 8 questions

    {
        "category": "city_8",
        "points": 10,
        "text": (
            "Where are we heading. We travel along the silver "
            "river's milonga shores and search for "
            "the city with the fresh air. "
            "The journey goes in a southern direction from one "
            "sunny land to another; the mustache is discounted here."
        ),
        "answer": "Buenos Aires",
    },
    {
        "category": "city_8",
        "points": 8,
        "text": (
            "The blue and yellow juniors bow and bocce, preferably "
            "flattening the competitor, named after the river. "
            "The country's history has been tumultuous, now it's "
            "calm, but there's still anger. "
            "The war was dirty, and the conflict was about the "
            "bird-rich island kingdom."
        ),
        "answer": "Buenos Aires",
    },
    {
        "category": "city_8",
        "points": 6,
        "text": (
            "The city's name is two-parted; here one says 'B' before 'A.' "
            "The president's palace is pink, and Elaine "
            "Paige's and Madonna's are white. "
            "The Spaniards arrived here in the 16th century, "
            "German refugees in 1945."
        ),
        "answer": "Buenos Aires",
    },
    {
        "category": "city_8",
        "points": 4,
        "text": (
            "With pomp and splendor, Fritiof Andersson dances with "
            "Carmencita in this cultural city where "
            "pears have had great significance. "
            "The Lord's fist saved the football-crazy land in '86. "
            "Today's star is not the Messiah but certainly the world's best."
        ),
        "answer": "Buenos Aires",
    },
    {
        "category": "city_8",
        "points": 2,
        "text": (
            "If Arjen Robben joins in, an enraged Tina turns into an anagram "
            "for both the capital of the country and the tango, "
            "situated by the shores of the Rio de la Plata."
        ),
        "answer": "Buenos Aires",
    },

    # City 9 questions

    {
        "category": "city_9",
        "points": 10,
        "text": (
            "On 10 points: Where are we heading? We leave the urban harbor "
            "city starting with 'D' and travel north towards "
            "a city on and with stripes. "
            "A man in a famous twelve with his cheerful message is a "
            "clue to this trade and finance center."
        ),
        "answer": "Johannesburg",
    },
    {
        "category": "city_9",
        "points": 8,
        "text": (
            "On 8 points: Gold discoveries drew people to our southern "
            "destination situated on the Witwatersrand plateau, "
            "where the continent's largest airport also lies."
        ),
        "answer": "Johannesburg",
    },
    {
        "category": "city_9",
        "points": 6,
        "text": (
            "On 6 points: F.W. de Klerk was born and "
            "baptized in this baptizer's city. "
            "If you know where Andres Iniesta shot home gold to the "
            "sound of vuvuzelas, you know which city we're seeking. "
            "The old country was a real voting land, but "
            "there were no free elections."
        ),
        "answer": "Johannesburg",
    },
    {
        "category": "city_9",
        "points": 4,
        "text": (
            "On 4 points: We continue into the old land of the Boers, "
            "where names like Kepler, Brahms, and Vermeer "
            "evoke thoughts of our million-city. "
            "Despite being the largest in the country, "
            "our city is not one of the country's three capitals."
        ),
        "answer": "Johannesburg",
    },
    {
        "category": "city_9",
        "points": 2,
        "text": (
            "On 2 points: It's true, Mandela wasn't divided here, but "
            "we've got our own version of separation anxiety, "
            "especially when it comes to "
            "our South African burg just south of Pretoria. "
            "We prefer unity, but our traffic jams seem to have other plans!"
        ),
        "answer": "Johannesburg",
    },

    # City 10 questions

    {
        "category": "city_10",
        "points": 10,
        "text": (
            "On 10 points: Where are we heading? We leave the twin city "
            "and set our sights on highs in lowland,"
            "where many goods have been, "
            "and in this growing country, the letter "
            "'F' in scarcity is yet to be found."
        ),
        "answer": "Singapore",
    },
    {
        "category": "city_10",
        "points": 8,
        "text": (
            "On 8 points: The land of Grimaldi is a good but long-distance "
            "clue because we are heading towards the "
            "lion city that is, in fact, a tiger. "
            "And here, monkeys also have a central significance."
        ),
        "answer": "Singapore",
    },
    {
        "category": "city_10",
        "points": 6,
        "text": (
            "On 6 points: Our finale is not as poor as one might think, and "
            "in this densely populated city, one can sip "
            "twisty drinks at Raffles Hotel."
        ),
        "answer": "Singapore",
    },
    {
        "category": "city_10",
        "points": 4,
        "text": (
            "On 4 points: We are now at the bottom of the Malacca "
            "Peninsula, and in this old British Crown Colony, "
            "there is one of the world's largest ports."
        ),
        "answer": "Singapore",
    },
    {
        "category": "city_10",
        "points": 2,
        "text": (
            "On 2 points: We have now arrived in a Southeast Asian mini- "
            "state, where the Merlion is a popular tourist attraction."
        ),
        "answer": "Singapore",
    },
]

